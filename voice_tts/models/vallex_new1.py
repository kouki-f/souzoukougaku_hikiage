...
class VALLE(VALLF):
    ...
    def inference(
        ...
        # 事前計算: 
        prompt_language_id = torch.LongTensor(np.array([self.language_ID[prompt_language]])).to(device)
        if isinstance(text_language, str):
            text_language_id = torch.LongTensor(np.array([self.language_ID[text_language]])).to(device)
        elif isinstance(text_language, List):
            text_language_id = torch.LongTensor(np.array([self.language_ID[tl] for tl in text_language])).to(device)
