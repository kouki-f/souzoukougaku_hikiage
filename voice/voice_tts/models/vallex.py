...
class VALLE(VALLF):
    ...
    def inference(
        ...
        device = x.device  # デバイス情報を取得

        # 事前計算: 
        prompt_language_id = torch.LongTensor([self.language_ID[prompt_language]]).to(device)
        if isinstance(text_language, str):
            text_language_id = torch.LongTensor([self.language_ID[text_language]]).to(device)
        elif isinstance(text_language, list):
            text_language_id = torch.LongTensor([self.language_ID[tl] for tl in text_language]).to(device)
        ...
        # 変数の初期化 (不要な繰り返しを避ける)
        text_len = x_lens.max().item()
        prompts = y
        prefix_len = y.shape[1]
        x_len = x_lens.max().item()
        x_attn_mask = torch.zeros((x_len, x_len), dtype=torch.bool)
        kv_cache = None
        use_kv_caching = True  # CPU処理のためキャッシングを無効化
        ...
        # Non-AR Decoders (条件分岐を1つにまとめ、不要な計算を避ける)
        if self.num_quantizers > 1:  
            y_emb = self.nar_audio_embeddings[0](y[:, int(self.ar_audio_prepend_bos) :])
            if self.prefix_mode in [2, 4]:  # Exclude enrolled_phonemes
                enrolled_len = enroll_x_lens.max().item()
                text = torch.cat([text[:, :1], text[:, enrolled_len - 1 :]], dim=1)
                text_len -= (enrolled_len - 2)
            ...
            for j in range(1, self.num_quantizers):  # prefix_mode に関係なく共通処理
